              ...
              {editingId === pin.callId ? (
                ...
                <>
                  <div className="truncate flex-grow">
                    <p className="font-semibold text-base text-gray-100 truncate" title={pin.alias || pin.callId}>
                        {pin.alias || pin.callId}
                    </p>
                    {canCallDirectly && (
                        status ? (
                            <div className="group relative text-xs flex items-center gap-1.5" tabIndex={0} aria-label={isOnline ? 'User is online' : `User is offline. Last seen ${formatTimeAgo(status.lastChanged)}`}>
