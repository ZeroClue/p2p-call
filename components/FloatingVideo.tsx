...
const MutedIcon: React.FC<React.SVGProps<SVGSVGElement> & { title?: string }> = ({
  title,
  ...props
}) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}>
    ...
    <path
      fillRule="evenodd"
      d="M2.023 2.023a.75.75 0 0 1 1.06 0L21.977 20.92a.75.75 0 1 1-1.06 1.06L2.023 3.083a.75.75 0 0 1 0-1.06Z"
      clipRule="evenodd"
    />
  </svg>
);
...
// FIX: Destructure title prop to render a <title> element for tooltips, which is better for accessibility.
// FIX: Added `& { title?: string }` to the component's props type to allow passing a `title` prop for accessibility.
const VideoIsOffIcon: React.FC<React.SVGProps<SVGSVGElement> & { title?: string }> = ({
  title,
  ...props
}) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}>
...
const ConnectionStatusIndicator: React.FC<{
  ...
  connectionState: RTCPeerConnectionState;
  isE2EEActive: boolean;
}> = ({ callState, connectionState, isE2EEActive }) => {
  ...
  return (
    <div
      ...
      title={`Connection Status: ${text}${isE2EEActive ? ' (End-to-End Encrypted)' : ''}`}
    >
...
const SignalBarsIcon: React.FC<{
  level: 'good' | 'average' | 'poor' | 'unknown';
  className?: string;
}> = ({ level, className }) => {
  const levelMap = {
  ...
  return (
    ...
    >
      <path
        d="M21 4.25a.75.75 0 0 1 .75.75v13.5a.75.75 0 0 1-1.5 0V5a.75.75 0 0 1 .75-.75Z"
        opacity={bars >= 4 ? 1 : 0.3}
      />
      <path
        d="M16.5 7.25a.75.75 0 0 1 .75.75v10.5a.75.75 0 0 1-1.5 0V8a.75.75 0 0 1 .75-.75Z"
        opacity={bars >= 3 ? 1 : 0.3}
      />
      <path
        d="M12 10.25a.75.75 0 0 1 .75.75v7.5a.75.75 0 0 1-1.5 0v-7.5a.75.75 0 0 1 .75-.75Z"
        opacity={bars >= 2 ? 1 : 0.3}
      />
      <path
        d="M7.5 13.25a.75.75 0 0 1 .75.75v4.5a.75.75 0 0 1-1.5 0v-4.5a.75.75 0 0 1 .75-.75Z"
...
const CallQualityIndicator: React.FC<{ stats: CallStats }> = ({ stats }) => {
  ...
  return (
    <div
      className="group relative flex items-center gap-2 text-sm text-white border-l border-gray-600 pl-3"
      title={`Call Quality: ${text}`}
    >
      ...
      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-48 p-3 bg-gray-900/90 backdrop-blur-sm border border-gray-700 rounded-lg text-xs text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 font-mono shadow-lg">
        <h4 className="font-bold text-center mb-2 border-b border-gray-700 pb-1 text-white">
          Connection Details
        </h4>
...
const FloatingVideo: React.FC<FloatingVideoProps> = ({
  ...
  const interactionStartRef = useRef({
    ...
    y: 0, // mouse position
    posX: 0,
    posY: 0, // element position
    width: 0,
    height: 0, // element size
  });
      ...
      >
        ...
        {(isRemoteMuted || isRemoteVideoOff) && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center gap-6 pointer-events-none">
            {isRemoteMuted && (
              <MutedIcon className="w-16 h-16 text-white/90" title="Peer is muted" />
            )}
            {isRemoteVideoOff && (
              <VideoIsOffIcon className="w-16 h-16 text-white/90" title="Peer video is off" />
            )}
          </div>
        )}
      </div>
