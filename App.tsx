...
import { CallState, PinnedEntry, ChatMessage } from './types';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'new' | 'recent' | 'pinned' | 'tools' | 'about'>(
    'new',
  );
  ...
  const {
    incomingCall,
    handleAcceptCall: handleAcceptCallIncoming,
    handleDeclineCall: handleDeclineCallIncoming,
  } = incomingCallHook;
  ...
  const handleCallPinned = useCallback(
    (pin: PinnedEntry) => {
      if (pin.peerId) {
        setPeerToRing(pin);
        enterLobby();
      ...
      case 'recent':
        return (
          <CallHistory
            ...
            onUpdateAlias={callHistory.updateHistoryAlias}
            onTogglePin={callHistory.togglePin}
            pinnedIds={new Set(pinned.map((p) => p.callId))}
            onDelete={callHistory.deleteHistory}
          />
        );
      case 'pinned':
        return (
          <PinnedCalls
            ...
            onCall={handleCallPinned}
            onUpdateAlias={callHistory.updatePinAlias}
            onUnpin={callHistory.unpin}
          />
        );
      case 'tools':
        return (
          <Tools
            userId={userId}
            onRestore={callHistory.restoreData}
          ...
          {isConfigError && (
            <div className="bg-gray-900/50 rounded-lg p-4 mb-6 text-left">
              ...
              <ol className="text-sm text-gray-300 space-y-2 list-decimal list-inside">
                ...
                <li>
                  Navigate to <strong>Authentication</strong> → <strong>Sign-in method</strong>
                </li>
                <li>
                  Enable <strong>Anonymous</strong> authentication
                </li>
  ...
  return (
    <CallContext.Provider value={callContextValue}>
      <div className="min-h-screen text-slate-200">
          ...
          <div className="relative container mx-auto px-4 py-8 md:py-16 flex flex-col items-center gap-10">
            <div className="text-center">
              ...
              <p className="text-gray-400 mt-2">
                Connect directly with anyone, anywhere. No accounts, no tracking, just a private
        ...
        {callState === CallState.MEDIA_ERROR && (
          <MediaErrorScreen errorMessage={errorMessage} onRetry={reset} />
        )}
        {callState === CallState.INCOMING_CALL && incomingCall && (
          <IncomingCallScreen
        ...
        {callState === CallState.LOBBY && (
          <div className="w-full h-screen flex items-center justify-center p-4">
            <Lobby
              ...
              onEnableE2EEChange={setEnableE2EE}
            />
          </div>
        )}
        ...
        {(callState === CallState.CONNECTED || callState === CallState.RECONNECTING) && (
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm overflow-hidden">
            ...
            />
            <LocalVideoPreview stream={localStream} isMuted={isMuted} isVideoOff={isVideoOff} />
            <Controls
              ref={controlsRef}
